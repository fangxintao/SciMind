...
class LlamaModel(BaseModel):
    ...
    def __init__(self,
        ...
        # fangxt special for kv
        # self.freqs_cos_kv, self.freqs_sin_kv, self.swap_mask_kv = precompute_freqs_cis_for_kv(
        #     config.hidden_size // config.num_heads, config.seq_length, dtype=config.rotary_dtype,
        #     pretrain_seqlen=config.pretrain_seqlen, extend_method=config.extend_method)
    ...
    def construct(self, tokens: Tensor, molecular_mask:Tensor, input_position=None, init_reset=True, batch_valid_length=None):
        """
        ...
        # print('llama.py 223行:', tokens.shape)
        if self.is_first_iteration:
            ...
            # ######fangxt change
            # freqs_cis_kv = (self.tile(self.reshape(self.freqs_cos_kv, (1, 1, seq_len, -1)), (bs, 1, 1, 1)),
            #  self.tile(self.reshape(self.freqs_sin_kv, (1, 1, seq_len, -1)), (bs, 1, 1, 1)),
            #  self.swap_mask_kv)
            
            # print('llama.py 228行:', freqs_cis)
            ...
            freqs_cis = (self.reshape(self.gather_past(self.freqs_cos, cur_pos, 0), (bs, 1, seq_len, -1)),
                         ...
                         self.swap_mask)
            # ######fangxt change
            # freqs_cis_kv = (self.reshape(self.gather_past(self.freqs_cos_kv, cur_pos, 0), (bs, 1, seq_len, -1)),
            #  self.reshape(self.gather_past(self.freqs_sin_kv, cur_pos, 0), (bs, 1, seq_len, -1)),
            #  self.swap_mask_kv)
        ...
        for i in range(self.num_layers):
            h, _ = self.layers[i](h, freqs_cis, molecular_mask, mask,
...
class LlamaForCausalLM(BaseModel):
    ...
    def __init__(self, config: LlamaConfig = None):
        ...
        # self.molecular_mask = Tensor(np.random.randint(0, 2, (256, 4 *16)), dtype=mstype.bool_)

    def prepare_inputs_for_generation(self, input_ids, **kwargs):

        # molecular_mask = np.where(input_ids > 32000, 1, 0)
        # print('molecular_mask:', molecular_mask)
        
        return {
            "input_ids": Tensor(input_ids, mstype.int32),
            # "molecular_mask": Tensor(molecular_mask, mstype.int32)
        }

    # pylint: disable=W0613
    def construct(self, input_ids, molecular_mask, labels=None, input_position=None, position_ids=None, attention_mask=None,
        ...
        """
        # print(molecular_mask)
        bsz, seqlen = input_ids.shape
        # print('llama.py 354行:', input_ids.shape)
        ...
        output = self.model(tokens, molecular_mask, input_position, init_reset, batch_valid_length)
