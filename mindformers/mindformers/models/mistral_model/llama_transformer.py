...
from mindformers.models.llama.llama_layer import LlamaFeedForward, LlamaRMSNorm, LlamaRotaryEmbedding, RotaryEmbedding
...
class LLamaAttention(nn.Cell):
    ...
    def __init__(self,
        ...
        # this is a new RotaryEmbbedding
       
        self.apply_rotary_emb = LlamaRotaryEmbedding( self.hidden_size, rotary_dtype)
    ...
    def construct(self, x: Tensor, freqs_cis: Tuple[Tensor, Tensor], mask=None,
                  key_past=None, value_past=None, batch_valid_length=None, freqs_cis_kv=None):
...
class LLamaDecodeLayer(nn.Cell):
    ...
    def construct(self, x, freqs_cis, molecular_mask, mask=None, init_reset=True, batch_valid_length=None):
        """ Forward of transformer block. """
        # print('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&', freqs_cis)
        self._check_input(x, freqs_cis, mask, init_reset, batch_valid_length)
        # fangxt change
        # self._check_input(x, freqs_cis_kv, mask, init_reset, batch_valid_length)
        ...
        # [bs, seq/1, hidden_dim] or [bs * seq/1, hidden_dim]
        ffn_out = self.feed_forward(ffn_norm, molecular_mask)
