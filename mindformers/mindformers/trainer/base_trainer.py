...
class BaseTrainer:
    ...
    def training_process(
        ...
        logger.info(f'.... config.load_checkpoint {config.load_checkpoint}')
        logger.info(f'.... config.only_save_strategy {config.only_save_strategy}')
        logger.info(f'.... config.resume_training {config.resume_training}')
    ...
    def predict_process(self,
        ...
        if not self.pipeline_task:
            ...
            print('********base_trainer的773行')
            need_add_tokens = open('/home/ma-user/work/Scimind/smiles_alphabet.txt', 'r').readlines()
            need_add_tokens_list = [x.strip() for x in need_add_tokens]
            tokenizer.add_tokens(need_add_tokens_list, special_tokens=True)
            ...
            print('xxxxxxxxxxxxxxxx', tokenizer.tokenize('{|C|}{|C|}'))
